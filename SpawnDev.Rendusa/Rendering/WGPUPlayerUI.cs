...
public class WGPUPlayerUI
{
    ...
    private void ComputeLayout(PlayerState state)
    {
        ...
        // Pin controls (prevent auto-hide)
        rx -= ButtonSize + Padding * 0.5f;
        var pinIcon = _renderer.State.ControlsPinned ? "📌" : "📍";
        _buttons.Add(new UIButton { Id = "pin", Icon = pinIcon, X = rx, Y = y, Width = ButtonSize, Height = ButtonSize, Active = _renderer.State.ControlsPinned, Opacity = _opacity });
    ...
    public PlayerAction? OnMouseUp(float normX, float normY)
    {
        ...
        else if (_clickTarget is UIButton clickedButton)
        {
            ...
            if (clickedButton.Enabled && clickedButton.HitTest(mx, my) != null) // Check if mouse is still over the button
            {
                ...
                // Pin toggle — prevent controls auto-hide
                else if (clickedButton.Id == "pin")
                {
                    _renderer.State.ControlsPinned = !_renderer.State.ControlsPinned;
                    _renderer.Invalidate();
                }
